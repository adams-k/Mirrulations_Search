...
def test_opensearch_endpoint_returns_status(client):
    """Test whether the opensearch endpoint returns a 200 OK status code."""
    search_term = 'Tribal Self-governance'
    response = client.get(f'/opensearch?term={search_term}')
    assert response.status_code == 200

def test_opensearch_endpoint_returns_status_code_400(client):
    """Test whether the opensearch endpoint returns a 400 Bad Request status code
